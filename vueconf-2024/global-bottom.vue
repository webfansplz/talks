...
const devtoolsVisible = computed(() => (formatter.value.devtools || false))
...
onMounted(() => {
  ...
  watchEffect(() => {
    const el = document.querySelector('.vue-devtools__panel') as HTMLElement
    const iframe = document.querySelector('.vue-devtools-frame') as HTMLElement
    if (el) {
      el.style.display = devtoolsVisible.value ? 'flex' : 'none'
      iframe.style.display = devtoolsVisible.value ? 'block' : 'none'
    }
  })
})
...
<template>
  <span absolute pointer-events-none rounded-full z--1 bg-gradient-to-r from-green-400 via-teal-400 to-blue-500 op75
